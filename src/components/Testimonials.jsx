  ...
  return (
    <section id="depoimentos" className="py-16 md:py-20 bg-slate-900/50">
      <div className="container mx-auto px-4 md:px-6">
        <h2 className="text-2xl md:text-3xl font-bold text-slate-100 mb-12 text-center">
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          {depoimentos.map((dep) => (
            <div
              key={dep.id}
              className="bg-gradient-to-br from-slate-800 to-slate-800/50 p-6 md:p-8 rounded-2xl border border-slate-700 relative"
            >
              ...
              <div className="flex items-center justify-center gap-4">
                ...
                <div className="text-left">
                  <p className="text-white font-bold text-sm md:text-base">
                    {dep.cargo ? `${dep.cargo} — ` : ''}{dep.autor}
                  </p>
                  {dep.empresa && (
                    <p className="text-emerald-400 text-xs md:text-sm">{dep.empresa}</p>
                  )}
                </div>
              </div>
            </div>
